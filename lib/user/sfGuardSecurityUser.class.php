...
class sfGuardSecurityUser extends sfBasicSecurityUser
{
    ...
    public function signIn( $user, $remember = false )
    {
        ...
        if ( $remember )
        {
            ...
            Doctrine::queryFrom( 'sfGuardRememberKey' )->delete( 'sfGuardRememberKey' )->where( 'sfGuardRememberKey.created_at < ?', $expiration_time_value )->execute();

            // remove other keys from this user
            Doctrine::queryFrom( 'sfGuardRememberKey' )->delete( 'sfGuardRememberKey' )->where( 'sfGuardRememberKey.user_id = ?', $user->getId() )->execute();
